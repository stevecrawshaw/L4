...
<https://mine.quarto.pub/hello-quarto/#/hello-quarto-title> Guardrails, YAML completion, python, julia

quarto can render rmarkdown

use format: in yaml instead of output: format: pdf - appears in viewer not pop up use #\| (hash pipe) in the chunk for chunk options
...
change to index.qmd create a yaml file in rstudio

\_quarto.yml

website: title: quarto navbar: left: - index.qmd - hello.qmd - python_notebook.ipynb \# to include a python notebook in a rendered qmd

execute: freeze: true #(controls computation - only run if instructed)
...
Content is: - reprodicible - portable
...
Application - NASA migrating data platform to cloud. NASA + Openscapes. Collaboration entailed need to embrace python. 2021 cloud hackathon - transition earthdata workflows to cloud can use multople ide's to collaborate
...
`chalkboard: true` for drawing on screen `code-line-numbers: "|1-2|3"` to animate code lines on screen
