...
- Confirm optimal approach to building a robust and reproducible analytic pipeline **[B4]**
...
In order to calculate this statistic, the hourly continuous data for each instrument are retrieved from the database and a function is iterated over the complete dataset to derive the number and percentage of missing observations. The function to retrieve data is not shown for brevity.
...
The code below sets up a database connection by retrieving the connection parameters from a config.yml file. The config file is added to .gitignore to ensure that credentials are not published on GitHub. **[S1]**
