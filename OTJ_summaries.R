...
sum_period_fnc <- function(period_vec) {
    ...
    out_hours <- period@day * 24 + period@hour
    out_mins <- period@minute
    glue("{out_hours}H {out_mins}M")
}
...
get_gt_table <- function(learning_date, OTJ_only = TRUE) {
    ...
    learning_clean_tbl <- learning_raw_tbl %>%
        ...
        mutate(
            hours = lubridate::hms(time_spent_on_activity_hh_mm),
            #period
            ksb = str_replace_all(
                select_ks_bs_that_apply,
                # make CR/LF
                pattern = ", ",
                replacement = "<br>"
            )
    ...
    new_names <-
        c(
            "date_of_learning_activity",
            "hours",
            "description_of_activity",
            "outcome_of_activity_roi",
            "ksb",
    ...
    month_gt <- learning_clean_tbl %>%
        ...
        summary_rows(
            ...
            # this formatter used for
            pattern = "{x}"
        ) %>% #character output where function makes char
...
get_gt_table(learning_date, OTJ_only = FALSE) %>%
    gtsave(
        filename = glue("learning_record_table_{learning_date}.png"),
        vwidth = 1920
    )
